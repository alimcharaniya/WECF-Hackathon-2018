...
const SEND_PRESCRIPTION_ID: u16 = 3;

const DOCTOR_CREATE_WALLET_ID: u16 = 4;
...
encoding_struct! {
    struct DoctorWallet {
        ...
        field pub_key:            &PublicKey  [00 => 32]
        field name:               &str        [32 => 40]
        field drug:               &str         [40 => 48]
    }
}

/// Add methods to the `Wallet` type for changing balance.
impl DoctorWallet {
    //no doctor wallet implementation
}
encoding_struct! {
    struct PharmaWallet {
        const SIZE = 48;
        ...
        field pub_key:            &PublicKey  [00 => 32]
        field name:               &str        [32 => 40]
        field currentOrder:       &str         [40 => 48]
    }
}

/// Add methods to the `Wallet` type for changing balance.
impl PharmaWallet {
    pub fn updateCurrentOrder(self, newDrug: &str) -> Self {
        let newCurOrder = self.currentOrder();
        Self::new(self.pub_key(), self.name(), self.currentOrder())
    }
}

// // // // // // // // // // DATA LAYOUT // // // // // // // // // //

pub struct ProviderSchema<'a> {
    view: &'a mut Fork,
}

impl<'a> ProviderSchema<'a> {
...
message! {
    struct DoctorCreateWallet {
        const TYPE = SERVICE_ID;
        const ID = DOCTOR_CREATE_WALLET_ID;
        const SIZE = 48;
...
/// Doctor requests prescription from pharama between the wallets.
message! {
    struct SendPrescriptionRequest {
        const TYPE = SERVICE_ID;
        const ID = SEND_PRESCRIPTION_ID;
        const SIZE = 104;

        field doctor:        &PublicKey  [00 => 32]
        field pharmacy:     &PublicKey  [32 => 64]
        field drug:          &str  [64 => 72]
        field pub_key:       &PublicKey  [72 => 104]
...
/// Execute a transaction.
impl Transaction for DoctorCreateWallet {
    /// Verify integrity of the transaction by checking the transaction
    /// signature.
    fn verify(&self) -> bool {
        self.verify_signature(self.pub_key())
...
/// The structure returned by the REST API for doctor -> pharama .
#[derive(Serialize, Deserialize)]
pub struct DoctorToPharmaResponse {
    pub tx_hash: Hash
}
...
impl CryptocurrencyApi {
    ...
    /// Common processing for transaction-accepting endpoints.
    fn post_request_prescription<T>(&self, req: &mut Request) -> IronResult<Response>
    where
        T: Transaction + Clone + for<'de> Deserialize<'de>,
    {
        println!("Im here !");
...
impl Api for CryptocurrencyApi {
    fn wire(&self, router: &mut Router) {
        ...
        let self_ = self.clone();
        let request_perscription = move |req: &mut Request| self_.post_request_prescription::<SendPrescriptionRequest>(req);
        let self_ = self.clone();
        let post_create_doctor_wallet = move |req: &mut Request| self_.post_transaction::<DoctorCreateWallet>(req);
        ...
        router.post("/v1/wallets/createDoctor", post_create_doctor_wallet, "post_create_doctor_wallet");
        router.post("/v1/wallet/requestPerscription", request_perscription, "request_perscription");
